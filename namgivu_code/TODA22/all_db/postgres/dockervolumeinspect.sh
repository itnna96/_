  # eg s230922_2155_postgres_postgres_v
docker volume inspect postgres_postgres_v
echo
docker volume inspect  postgres_postgres_v | grep 'Mountpoint'
docker volume inspect  postgres_postgres_v | grep 'Mountpoint' | cut -d':' -f2 | cut -d'"' -f2
